...
function normalizeNameForLookup(v) { return String(v || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim().replace(/\s+/g, " "); }
...
async function readPartnerLite(odoo, partnerId) {
  ...
  if (!id) return null;
  try {
    const rows = await odoo.executeKw("res.partner", "read", [[id]], { fields: ["id", "name", "email", "phone", "mobile", "street", "city"] });
    return rows?.[0] || null;
  } catch { return null; }
}
...
async function getCreatorDisplayData(createdByUserId) {
  try {
    ...
    const row = r.rows?.[0] || {};
    return { full_name: normalizeSellerDisplayName(row.full_name), username: normalizeSellerDisplayName(row.username) };
  } catch { return { full_name: "", username: "" }; }
}
...
async function hydrateMeasurementLinePrices(odoo, payload, seeds) {
  ...
  if (templateIds.length) {
    try { templates = await odoo.executeKw("product.template", "read", [templateIds], { fields: ["id", "list_price"] }); } catch {}
  }
  const tmplById = new Map((templates || []).map((t) => [Number(t.id), t]));
  return list.map((seed) => {
    const p = byId.get(Number(seed.product_id));
    const tmpl = tmplById.get(Number(toIntId(p?.product_tmpl_id)));
    return {
      ...seed,
      ...
      basePrice: Number(p?.lst_price ?? p?.list_price ?? tmpl?.list_price ?? seed?.basePrice ?? 0) || 0,
    };
  });
}
...
async function getOrCreateRevisionQuote(originalQuote, finalLines) {
  ...
  const ins = await dbQuery(`insert into public.presupuestador_quotes
      (quote_kind, parent_quote_id, created_by_user_id, created_by_role, fulfillment_mode, pricelist_id, bill_to_odoo_partner_id, end_customer, lines, payload, note, catalog_kind, status, commercial_decision, technical_decision, final_status)
     values ('copy', $1, $2, $3, $4, $5, $6, $7::jsonb, $8::jsonb, $9::jsonb, $10, $11, 'draft', 'pending', 'pending', 'draft')
     returning *`,
    [originalQuote.id, originalQuote.created_by_user_id, originalQuote.created_by_role, originalQuote.fulfillment_mode, originalQuote.pricelist_id, originalQuote.bill_to_odoo_partner_id, JSON.stringify(originalQuote.end_customer || {}), JSON.stringify(finalLines), JSON.stringify(originalQuote.payload || {}), originalQuote.note || null, originalQuote.catalog_kind || "porton"]
  );
...
async function syncFinalQuoteToOdoo({ odoo, revisionQuote, originalQuote, approverUser }) {
  ...
  const createdOrderId = await odoo.executeKw("sale.order", "create", [{ partner_id: partnerId, pricelist_id: pricelistId, order_line: orderLines, origin: referenceNv, client_order_ref: referenceNv, note }]);
  ...
  return { order, metrics: { detailed_total: grossPositiveTotal, advance_discounted_amount: round2(Math.max(0, grossPositiveTotal - Math.max(0, totalToCharge))), tolerance_percent: tolerancePercent, tolerance_amount: toleranceAmount, difference_amount: rawDifference, absorbed_by_company: absorbedByCompany, final_amount_to_charge: round2(Math.max(0, totalToCharge)), reference_nv: referenceNv } };
}
