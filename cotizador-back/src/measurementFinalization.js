...
function calcDetailedUnitWithIva(line, payload) {
  ...
  const base =
    Number(line?.basePrice ?? line?.base_price ?? line?.price ?? 0) || 0;
...
function buildMeasurementLineSeedsFromTechnicalRules(
  originalQuote,
  form,
  rules,
) {
  ...
  for (const rule of Array.isArray(rules) ? rules : []) {
    ...
    out.push({
      ...
      qty: 1,
      name: String(
        rule.product_label || rule.name || `Producto ${rule.product_id}`,
      ).trim(),
      raw_name: String(
        rule.product_label || rule.name || `Producto ${rule.product_id}`,
      ).trim(),
  ...
  for (const field of Array.isArray(fields) ? fields : []) {
    ...
    if (bindingType === "custom_product") {
      ...
      out.push({
        ...
        qty: 1,
        name: String(
          field?.odoo_product_label || field?.label || `Producto ${productId}`,
        ).trim(),
        raw_name: String(
          field?.odoo_product_label || field?.label || `Producto ${productId}`,
        ).trim(),
...
function dedupeLines(lines) {
  ...
  for (const line of Array.isArray(lines) ? lines : []) {
    ...
    if (!id) continue;
    if (!map.has(id))
      map.set(id, { ...line, qty: Number(line?.qty || 1) || 1 });
    else {
...
async function hydrateMeasurementLinePrices(odoo, payload, seeds) {
  ...
  const ids = [
    ...new Set(list.map((l) => Number(l.product_id)).filter(Boolean)),
  ];
  ...
  return list.map((seed) => {
    ...
    return {
      ...seed,
      name:
        toText(seed?.name) || toText(p?.name) || `Producto ${seed.product_id}`,
      raw_name:
        toText(seed?.raw_name) ||
        toText(p?.name) ||
        `Producto ${seed.product_id}`,
      code: toText(p?.default_code) || seed?.code || null,
      basePrice:
        Number(p?.lst_price ?? p?.list_price ?? seed?.basePrice ?? 0) || 0,
    };
  });
}
function buildDiscountPreviewLine({
  originalQuote,
  pricedLines,
  tolerancePercent,
}) {
  ...
  const toleranceAmount = round2(
    (originalBudgeted * Number(tolerancePercent || 0)) / 100,
  );
  ...
  const reference =
    referenceNumberFromQuote(originalQuote, null) ||
    toText(originalQuote?.quote_number) ||
    "ANTICIPO";
  return {
...
async function syncFinalQuoteToOdoo({ odoo, revisionQuote, originalQuote }) {
  ...
  const productIds = [
    ...new Set(
      lines
        ...
        .filter(Boolean)
        .concat([Number(PLACEHOLDER_PRODUCT_ID)]),
    ),
  ];
  ...
  for (const l of lines) {
    ...
    const priceUnit =
      typeof l?.price_unit === "number"
        ? round2(l.price_unit)
        : calcDetailedUnitWithIva(
            l,
            revisionQuote.payload || originalQuote.payload || {},
          );
    totalToCharge = round2(totalToCharge + qty * priceUnit);
    if (priceUnit > 0)
      grossPositiveTotal = round2(grossPositiveTotal + qty * priceUnit);
    orderLines.push([
      0,
      0,
      {
  ...
  const rawDifference = round2(
    Math.max(0, grossPositiveTotal - originalBudgeted),
  );
  const absorbedByCompany =
    originalBudgeted > 0 && rawDifference <= toleranceAmount;
...
async function buildMeasurementFinalizationBase({ odoo, originalQuote, measurementForm }) {
  ...
  const fieldBindingSeeds = buildMeasurementLineSeedsFromFieldBindings(
    measurementForm || {},
    technicalFields.fields || [],
  );
  const mergedSeeds = dedupeLines([
    ...legacySeeds,
    ...technicalSeeds,
    ...fieldBindingSeeds,
  ]);
  const pricedPositiveLines = await hydrateMeasurementLinePrices(
    odoo,
...
export async function finalizeMeasurementToRevisionQuote({
  ...
  const updFinal = await dbQuery(
    `update public.presupuestador_quotes set status='synced_odoo', final_status='synced_odoo', final_sale_order_id=$2, final_sale_order_name=$3, final_synced_at=now(), final_tolerance_percent=$4, final_tolerance_amount=$5, final_difference_amount=$6, final_absorbed_by_company=$7 where id=$1 returning *`,
    [
      qSync.id,
      Number(order.id),
      order.name,
