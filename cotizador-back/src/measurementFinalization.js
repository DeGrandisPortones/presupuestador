...
function toIntId(v) { const x = toScalar(v); const n = Number(x); return Number.isFinite(n) ? n : null; }
...
function referenceNumberFromQuote(originalQuote, revisionQuote) {
  const direct = toText(originalQuote?.odoo_sale_order_name) || toText(originalQuote?.final_sale_order_name) || toText(originalQuote?.quote_number) || toText(revisionQuote?.final_sale_order_name) || toText(revisionQuote?.quote_number);
...
function buildRuleContext(originalQuote, form) {
  ...
  return {
    ...form,
    surface_m2: Number.isFinite(widthM) && Number.isFinite(heightM) ? round2(widthM * heightM) : 0,
...
function buildMeasurementLineSeedsFromLegacyMappings(form, rules) {
  ...
  for (const rule of Array.isArray(rules) ? rules : []) {
    ...
    for (const entry of values) {
      ...
      if (!matches) continue;
      out.push({ product_id: Number(entry.product_id), qty: 1, name: String(entry.product_label || entry.label || rule.field_label || rule.field_key || `Producto ${entry.product_id}`).trim(), raw_name: String(entry.product_label || entry.label || rule.field_label || rule.field_key || `Producto ${entry.product_id}`).trim(), code: null, basePrice: 0 });
    }
  }
  return out;
}
function buildMeasurementLineSeedsFromTechnicalRules(originalQuote, form, rules) {
  ...
  for (const rule of Array.isArray(rules) ? rules : []) {
    ...
    out.push({ product_id: Number(rule.product_id), qty: 1, name: String(rule.product_label || rule.name || `Producto ${rule.product_id}`).trim(), raw_name: String(rule.product_label || rule.name || `Producto ${rule.product_id}`).trim(), code: null, basePrice: 0 });
  }
  return out;
}
...
function buildMeasurementLineSeedsFromFieldBindings(form, fields) {
  ...
  for (const field of Array.isArray(fields) ? fields : []) {
    ...
    if (String(field?.odoo_binding_type || "none") !== "product") continue;
    ...
    out.push({ product_id: productId, qty: 1, name: String(field?.odoo_product_label || field?.label || `Producto ${productId}`).trim(), raw_name: String(field?.odoo_product_label || field?.label || `Producto ${productId}`).trim(), code: null, basePrice: 0 });
  }
  return out;
}
function dedupeLines(lines) {
  ...
  for (const line of Array.isArray(lines) ? lines : []) {
    ...
    if (!map.has(id)) map.set(id, { ...line, qty: Number(line?.qty || 1) || 1 });
      ...
      map.set(id, { ...prev, qty: (Number(prev.qty || 1) || 1) + (Number(line?.qty || 1) || 1) });
    }
  }
  return [...map.values()];
}
async function hydrateMeasurementLinePrices(odoo, payload, seeds) {
  ...
  let products = [];
  try { products = await odoo.executeKw("product.product", "read", [ids], { fields: ["id", "name", "default_code", "uom_id", "lst_price", "list_price", "product_tmpl_id"] }); } catch {}
  const byId = new Map((products || []).map((p) => [Number(p.id), p]));
  return list.map((seed) => {
    const p = byId.get(Number(seed.product_id));
    return { ...seed, name: toText(seed?.name) || toText(p?.name) || `Producto ${seed.product_id}`, raw_name: toText(seed?.raw_name) || toText(p?.name) || `Producto ${seed.product_id}`, code: toText(p?.default_code) || seed?.code || null, basePrice: Number(p?.lst_price ?? p?.list_price ?? seed?.basePrice ?? 0) || 0 };
  });
}
function buildDiscountPreviewLine({ originalQuote, pricedLines, tolerancePercent }) {
  ...
  const positiveTotal = round2(pricedLines.reduce((acc, line) => { const price = calcDetailedUnitWithIva(line, originalQuote?.payload || {}); const qty = Number(line?.qty || 1) || 1; if (price <= 0) return acc; return acc + (qty * price); }, 0));
  ...
  return { product_id: PLACEHOLDER_PRODUCT_ID, qty: 1, name: `Descuento anticipo presupuesto ${reference}`, raw_name: `Descuento anticipo presupuesto ${reference}`, code: null, price_unit: round2(-discountAmount), basePrice: 0 };
}
async function getOrCreateRevisionQuote(originalQuote, finalLines) {
  const existing = await dbQuery(`select * from public.presupuestador_quotes where quote_kind='copy' and parent_quote_id=$1 order by created_at desc nulls last, id desc limit 1`, [originalQuote.id]);
  const copy = existing.rows?.[0];
  if (copy) {
    const upd = await dbQuery(`update public.presupuestador_quotes set lines=$2::jsonb, end_customer=$3::jsonb, payload=$4::jsonb, note=$5, final_status='draft' where id=$1 returning *`, [copy.id, JSON.stringify(finalLines), JSON.stringify(originalQuote.end_customer || {}), JSON.stringify(originalQuote.payload || {}), originalQuote.note || null]);
    return upd.rows?.[0] || copy;
  }
  const ins = await dbQuery(`insert into public.presupuestador_quotes (quote_kind, parent_quote_id, created_by_user_id, created_by_role, fulfillment_mode, pricelist_id, bill_to_odoo_partner_id, end_customer, lines, payload, note, catalog_kind, status, commercial_decision, technical_decision, final_status) values ('copy', $1, $2, $3, $4, $5, $6, $7::jsonb, $8::jsonb, $9::jsonb, $10, $11, 'draft', 'pending', 'pending', 'draft') returning *`, [originalQuote.id, originalQuote.created_by_user_id, originalQuote.created_by_role, originalQuote.fulfillment_mode, originalQuote.pricelist_id, originalQuote.bill_to_odoo_partner_id, JSON.stringify(originalQuote.end_customer || {}), JSON.stringify(finalLines), JSON.stringify(originalQuote.payload || {}), originalQuote.note || null, originalQuote.catalog_kind || "porton"]);
  return ins.rows?.[0] || null;
}
async function syncFinalQuoteToOdoo({ odoo, revisionQuote, originalQuote }) {
  const partnerId = toIntId(originalQuote?.bill_to_odoo_partner_id) || 1;
  ...
  for (const l of lines) {
    ...
    const priceUnit = typeof l?.price_unit === "number" ? round2(l.price_unit) : calcDetailedUnitWithIva(l, revisionQuote.payload || originalQuote.payload || {});
    ...
    orderLines.push([0, 0, { product_id: productId, product_uom_qty: qty, product_uom: uomId, name: toText(l?.raw_name || l?.name || p?.name), price_unit: priceUnit }]);
  }
  const originalBudgeted = round2(Number(originalQuote.deposit_amount || 0) || 0);
  ...
  const createdOrderId = await odoo.executeKw("sale.order", "create", [{ partner_id: partnerId, pricelist_id: toIntId(revisionQuote?.pricelist_id) || toIntId(originalQuote?.pricelist_id) || 1, order_line: orderLines, origin: referenceNv, client_order_ref: referenceNv }]);
  const order = { id: Number(createdOrderId), name: referenceNv };
  return { order, metrics: { detailed_total: grossPositiveTotal, tolerance_percent: tolerancePercent, tolerance_amount: toleranceAmount, difference_amount: rawDifference, absorbed_by_company: absorbedByCompany, final_amount_to_charge: round2(Math.max(0, totalToCharge)), reference_nv: referenceNv } };
}
export async function finalizeMeasurementToRevisionQuote({ odoo, originalQuote, measurementForm }) {
  ...
  if (!finalLines.length) return { revisionQuote: null, generated_lines: [], synced: false, reason: "Sin reglas aplicables" };
  const revisionQuote = await getOrCreateRevisionQuote(originalQuote, finalLines);
  if (!revisionQuote || !odoo) return { revisionQuote, generated_lines: finalLines, synced: false, reason: !odoo ? "Odoo no disponible" : "No se pudo crear la copia" };
  const updSync = await dbQuery(`update public.presupuestador_quotes set status='syncing_odoo', final_status='syncing_odoo', final_technical_decision='approved', final_logistics_decision='approved', final_technical_notes=null, final_logistics_notes=null where id=$1 returning *`, [revisionQuote.id]);
  const qSync = updSync.rows?.[0] || revisionQuote;
  const { order, metrics } = await syncFinalQuoteToOdoo({ odoo, revisionQuote: qSync, originalQuote });
  const updFinal = await dbQuery(`update public.presupuestador_quotes set status='synced_odoo', final_status='synced_odoo', final_sale_order_id=$2, final_sale_order_name=$3, final_synced_at=now(), final_tolerance_percent=$4, final_tolerance_amount=$5, final_difference_amount=$6, final_absorbed_by_company=$7 where id=$1 returning *`, [qSync.id, Number(order.id), order.name, metrics.tolerance_percent, metrics.tolerance_amount, metrics.difference_amount, metrics.absorbed_by_company]);
