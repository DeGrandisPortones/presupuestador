...
import { getCommercialFinalToleranceAreaM2, getTechnicalMeasurementRules, getTechnicalMeasurementFieldDefinitions } from "../settingsDb.js";
...
export function buildMeasurementsRouter(odoo = null) {
  ...
  router.put("/:id", requireMeasurementEditor, async (req, res, next) => {
    try {
      ...
      const forceSellerReturn =
        areaGuard.forced_return_to_seller === true || forceSellerReturnByItem18;
      if (submit && forceSellerReturn && !returnToSeller) {
        const item18Message =
          forceSellerReturnByItem18
            ? item18Change.message || "El cambio en el item 18 puede ocasionar costos adicionales y debe pasar al vendedor."
            : DEFAULT_RETURN_REASON;
        return res.status(400).json({
          ok: false,
          error: item18Message,
          force_return_to_seller: true,
          default_return_reason: item18Message,
      ...
      if (returnToSeller || forceSellerReturn) {
        ...
        return res.json({ ok: true, quote: upd.rows?.[0] || null, returned_to_seller: true, moved_to_seller: true, force_return_to_seller: forceSellerReturn, default_return_reason: areaGuard.default_return_reason, measurement_surface_guard: areaGuard });
      }
  ...
  router.post("/:id/return/confirm", async (req, res, next) => {
    try {
      ...
      const cur = await dbQuery(`select * from public.presupuestador_quotes where id=$1 limit 1`, [id]); const quote = cur.rows?.[0]; if (!quote) return res.status(404).json({ ok: false, error: "Presupuesto no encontrado" });
      const isOwner = String(quote.created_by_user_id) === String(u.user_id); if (!isOwner || !(u?.is_vendedor || u?.is_distribuidor)) return res.status(403).json({ ok: false, error: "No autorizado" });
      if (!quoteLooksLikeReturnedToSeller(quote)) return res.status(409).json({ ok: false, error: "El portón no está devuelto al vendedor" });
      ...
      const nextPricelistId = quoteUpdate?.pricelist_id !== undefined ? Number(quoteUpdate.pricelist_id || 0) || null : quote.pricelist_id;
      const nextBillToPartnerId = quoteUpdate?.bill_to_odoo_partner_id !== undefined ? (quoteUpdate.bill_to_odoo_partner_id ? Number(quoteUpdate.bill_to_odoo_partner_id) : null) : quote.bill_to_odoo_partner_id;
      ...
      const upd = await dbQuery(
        `update public.presupuestador_quotes
          ...
          where id=$1
          returning *`,
        [id, nextFulfillmentMode, nextPricelistId, nextBillToPartnerId, JSON.stringify(nextEndCustomer || {}), JSON.stringify(cleanedLines), JSON.stringify(nextPayload), nextNote, nextCatalogKind]
      );
