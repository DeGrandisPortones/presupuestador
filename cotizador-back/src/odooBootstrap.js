...
export async function loadOdooBootstrap(odoo) {
  ...
  const products = await odoo.executeKw(
    ...
    [[["sale_ok", "=", true]]],
    {
      fields: ["id", "name", "display_name", "default_code", "product_tmpl_id", "list_price"],
      limit: 5000,
      order: "name asc",
    }
  );
  ...
  if (tmplIds.length) {
    tmplRows = await odoo.executeKw(
      ...
      "search_read",
      [[["id", "in", tmplIds]]],
      { fields: ["id", "name", "display_name", "product_tag_ids"], limit: tmplIds.length }
    );
  }
  ...
  const tmplNames = new Map(
    tmplRows.map((t) => [
      Number(t.id),
      cleanText(t.display_name || t.name),
    ])
  );
