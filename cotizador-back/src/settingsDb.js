...
function normalizeTechnicalMeasurementRules(raw = {}) {
  ...
  return { rules: rules.map((r, i) => normalizeTechnicalMeasurementRule(r, i)).filter(Boolean).sort((a, b) => a.sort_order - b.sort_order) };
}
...
function normalizeTechnicalMeasurementField(field = {}, index = 0) {
  ...
  return {
    ...
    value_source_path: normalizeText(field?.value_source_path || field?.value_source_key || ''),
...
export async function setCommercialFinalQuoteSettings({ tolerance_percent }) {
  return setSetting(FINAL_QUOTE_SETTINGS_KEY, { tolerance_percent: normalizeTolerancePercent(tolerance_percent) });
}
