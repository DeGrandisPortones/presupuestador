...
export async function ensureCommercialConsultTables() {
  ...
  await dbQuery(`alter table public.presupuestador_commercial_tickets add column if not exists on_behalf_of_user_id bigint null references public.presupuestador_users(id);`);
  await dbQuery(`alter table public.presupuestador_commercial_ticket_messages add column if not exists attachment jsonb null;`);
  await dbQuery(`alter table public.presupuestador_commercial_tickets add column if not exists reference_number text null;`);
  ...
  await dbQuery(`create index if not exists presupuestador_commercial_tickets_on_behalf_of_idx on public.presupuestador_commercial_tickets(on_behalf_of_user_id);`);
  ...
  await dbQuery(`create index if not exists presupuestador_commercial_tickets_reference_number_idx on public.presupuestador_commercial_tickets(reference_number);`);
  ...
  return `
    select
      ...
      case
        ...
        when coalesce(creator.is_enc_comercial, false) or coalesce(creator.is_superuser, false) then 'enc_comercial'
        else 'usuario'
...
async function insertCommercialTicket(client, {
  ...
  const createdTicket = await client.query(
    `
      ...
      returning id
    `,
    [creatorUserId, targetUserId, assignedToUserId, status, subject, referenceNumber, requesterLastReadAt, commercialLastReadAt, now]
  );
...
export async function createCommercialConsult(user, { subject, message, target_user_id, target_user_ids, audience, attachment, reference_number } = {}) {
  await ensureCommercialConsultTables();
  const staffCreating = isCommercialUser(user);
  if (!staffCreating && !isRequesterUser(user)) {
    throw new Error("Solo vendedor, distribuidor o Enc. Comercial puede crear consultas comerciales");
  }
...
export async function addCommercialConsultMessage(user, id, { message, attachment } = {}) {
  await ensureCommercialConsultTables();
  const cleanMessage = normalizeMessage(message);
  if (!cleanMessage) throw new Error("Falta mensaje");
  const cleanAttachment = normalizeAttachment(attachment);
...
export async function closeCommercialConsult(user, id, { resolution, attachment } = {}) {
  await ensureCommercialConsultTables();
  ...
  await withTx(async (client) => {
    ...
    await client.query(
      `
        ...
        values ($1, $2, 'enc_comercial', $3, 'resolution', $4::jsonb, $5)
      `,
