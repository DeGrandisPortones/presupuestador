...
export const TECHNICAL_RULE_OPERATORS = [
  ...
  { value: ">=", label: ">=" }, { value: "<", label: "<" }, { value: "<=", label: "<=" }, { value: "contains", label: "contiene" },
];
...
export function parseOptions(raw) {
  if (Array.isArray(raw)) return raw.map((item) => typeof item === "object" ? { value: String(item.value || "").trim(), label: String(item.label || item.value || "").trim() } : { value: String(item || "").trim(), label: String(item || "").trim() }).filter((x) => x.value);
  return String(raw || "").split(",").map((x) => x.trim()).filter(Boolean).map((x) => ({ value: x, label: x }));
}
export function mergeMeasurementFields(customFields = []) {
  ...
  return [...byKey.values()].sort((a, b) => (Number(a.sort_order || 0) - Number(b.sort_order || 0)) || String(a.label || a.key).localeCompare(String(b.label || b.key), "es"));
}
