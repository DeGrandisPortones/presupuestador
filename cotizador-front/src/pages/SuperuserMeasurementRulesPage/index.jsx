...
export default function SuperuserMeasurementRulesPage() {
  ...
  const catalogQ = useQuery({
    queryKey: ["adminCatalogRulesProducts"],
    queryFn: () => adminGetCatalog("porton"),
    enabled: !!user?.is_superuser,
  });
  ...
  useEffect(() => {
    ...
    setFieldDraft({
      fields: (fieldsQ.data.fields || []).map((field, index) => normalizeFieldDraft(field, index)),
    });
  }, [fieldsQ.data]);
  ...
  const targetFieldOptions = allFields.filter(
    (item) => !["surface_m2", "budget_width_m", "budget_height_m"].includes(item.key)
  );
  ...
  return (
    <div className="container">
      <div className="card">
        ...
        <div className="muted">
          Desde acá creás campos nuevos para la planilla. Ejemplo: <b>automatización</b> o <b>posición de motor</b>.
        </div>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setFieldDraft({
                  fields: (saved.fields || []).map((field, index) => normalizeFieldDraft(field, index)),
                });
      ...
      <div className="card">
        ...
        <div className="muted">
          Acá definís cómo se comportan los campos. Ejemplo: si <b>automatización = no</b>, entonces <b>pasador_manual = si</b>. Si <b>automatización = si</b>, mostrar <b>posición de motor</b> y permitir <b>derecha, izquierda, doble</b>.
        </div>
        <div className="spacer" />
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Button
            onClick={() =>
              setRuleDraft((prev) => ({
                rules: [...(prev.rules || []), newRule((prev.rules || []).length + 1)],
              }))
            }
          >
            + Agregar regla
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setRuleDraft({
                  rules: (saved.rules || []).map((rule, index) => normalizeRuleDraft(rule, index)),
                });
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10 }}>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setRuleDraft((prev) => {
                        const next = [...(prev.rules || [])];
                        next[index] = { ...next[index], action_type: e.target.value };
                        return { rules: next };
                      })
                    }
                    style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ddd" }}
                ...
                <div>
                  ...
                  <Input
                    ...
                    style={{ width: "100%" }}
                    disabled={rule.action_type === "show_field" || rule.action_type === "hide_field"}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(v) =>
                      setRuleDraft((prev) => {
                        const next = [...(prev.rules || [])];
                        next[index] = { ...next[index], target_options_text: v };
                        return { rules: next };
                      })
                    }
                    style={{ width: "100%" }}
