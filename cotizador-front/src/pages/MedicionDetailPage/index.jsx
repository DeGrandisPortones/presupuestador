...
function buildBudgetContext(quote, catalog) {
  return {
    payload: quote?.payload || {},
    end_customer: quote?.end_customer || {},
    measurement_prefill: quote?.measurement_prefill || {},
    quote: { id: quote?.id || null, quote_number: quote?.quote_number || "", fulfillment_mode: quote?.fulfillment_mode || "", note: quote?.note || "", created_by_role: quote?.created_by_role || "" },
...
function resolveSectionBudgetValue(field, budgetContext) {
  ...
  const values = products.map((product) => key === "product_id" ? String(product.product_id || "") : String(product?.[key] || product?.display_name || "").trim()).filter(Boolean);
...
function coerceDynamicValue(field, value) {
  ...
  if (fieldType === "number") { const n = Number(String(value ?? "").replace(",", ".")); return Number.isFinite(n) ? String(n) : ""; }
  return String(value ?? "");
}
function resolveFieldAutofillValue(field, budgetContext) {
  ...
  if (sourceType === "budget_field") return getByPath(budgetContext, field?.value_source_path);
...
function renderDynamicSectionFields({ sectionKey, fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor }) {
  ...
  return <><div className="spacer" /><Row>{visibleFields.map((field) => { const fieldMeta = allFields.find((item) => item.key === field.key) || field; const allowed = dynamicUi.allowedOptions[field.key]; const value = getByPath(form, field.key); const disabled = !canEditDynamicField(fieldMeta, { isTechnical, isMedidor }); return <Field key={field.key} label={field.label}>{renderDynamicInput({ field: fieldMeta, value, allowedValues: allowed, disabled, onChange: (nextValue) => setForm((prev) => setByPath(prev, field.key, nextValue)) })}</Field>; })}</Row></>;
}
...
export default function MedicionDetailPage() {
  ...
  const catalogQ = useQuery({ queryKey: ["catalogBootstrapForMeasurement", "porton"], queryFn: () => getCatalogBootstrap("porton"), enabled: !!quoteId });
  ...
  const dynamicFields = useMemo(() => { const raw = Array.isArray(dynamicFieldsQ.data?.fields) ? dynamicFieldsQ.data.fields : []; return raw.filter((field) => field?.active !== false); }, [dynamicFieldsQ.data]);
  const allFields = useMemo(() => mergeMeasurementFields(dynamicFields), [dynamicFields]);
  const fieldsBySection = useMemo(() => { const out = {}; for (const field of dynamicFields) { const key = String(field?.section || "otros").trim().toLowerCase() || "otros"; if (!out[key]) out[key] = []; out[key].push(field); } return out; }, [dynamicFields]);
  const budgetContext = useMemo(() => buildBudgetContext(quote, catalogQ.data), [quote, catalogQ.data]);
  const dynamicUi = useMemo(() => { if (!form || !quote) return { hidden: new Set(), forcedValues: {}, allowedOptions: {} }; return evaluateDynamicRules({ form, quote, rules: dynamicRulesQ.data?.rules || [] }); }, [form, quote, dynamicRulesQ.data]);
  ...
  }, [form, dynamicFields, budgetContext]);
  ...
  useEffect(() => {
    ...
    for (const [fieldKey, fieldValue] of entries) {
      ...
      if (String(current ?? "") !== String(fieldValue ?? "")) { next = setByPath(next, fieldKey, fieldValue); changed = true; }
    }
    if (changed) setForm(next);
  ...
  return (
    <div className="container">
      <div className="card"><h2 style={{ marginTop: 0 }}>Planilla de medición / datos técnicos</h2><div className="muted">Presupuesto #{quote?.quote_number || quote?.odoo_sale_order_name || "—"}</div><div className="spacer" /><Button variant="ghost" onClick={() => navigate(-1)}>Volver</Button></div>
      <div className="spacer" />
      <Section title={SECTION_LABELS.datos_generales}><Row><Field label="Nota de Venta / NV"><Input value={form.nota_venta || ""} onChange={(v) => setForm({ ...form, nota_venta: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field><Field label="Fecha de Nota de Pedido"><Input type="date" value={form.fecha_nota_pedido || ""} onChange={(v) => setForm({ ...form, fecha_nota_pedido: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field><Field label="Fecha de medición"><Input type="date" value={form.fecha || ""} onChange={(v) => setForm({ ...form, fecha: v })} style={{ width: "100%" }} disabled={!isTechnical && !isMedidor} /></Field><Field label="Distribuidor"><Input value={form.distribuidor || ""} onChange={(v) => setForm({ ...form, distribuidor: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field></Row><div className="spacer" /><Row><Field label="Nombre del cliente"><Input value={form.cliente_nombre || ""} onChange={(v) => setForm({ ...form, cliente_nombre: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field><Field label="Apellido del cliente"><Input value={form.cliente_apellido || ""} onChange={(v) => setForm({ ...form, cliente_apellido: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field><Field label="Alto final (mm)"><Input value={form.alto_final_mm || ""} onChange={(v) => setForm({ ...form, alto_final_mm: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field><Field label="Ancho final (mm)"><Input value={form.ancho_final_mm || ""} onChange={(v) => setForm({ ...form, ancho_final_mm: v })} style={{ width: "100%" }} disabled={!isTechnical} /></Field></Row>{renderDynamicSectionFields({ sectionKey: "datos_generales", fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor })}</Section>
      <Section title={SECTION_LABELS.esquema_medidas}><div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-start" }}><div style={{ flex: 2, minWidth: 320 }}><div style={{ border: "1px solid #eee", borderRadius: 12, padding: 10, background: "#fff" }}><div style={{ position: "relative", width: "100%" }}><img src="/measurement_scheme.png" alt="Esquema de medición" style={{ width: "100%", height: "auto", display: "block" }} />{SCHEME_RECT_PCTS.alto.map((p, i) => { const v = form.esquema?.alto?.[i]; if (!v) return null; return <div key={`alto-ov-${i}`} style={{ ...schemeOverlayBaseStyle, left: `${p.left}%`, top: `${p.top}%`, width: `${p.width}%`, height: `${p.height}%`, fontSize: 14 }}>{v}</div>; })}{SCHEME_RECT_PCTS.ancho.map((p, i) => { const v = form.esquema?.ancho?.[i]; if (!v) return null; return <div key={`ancho-ov-${i}`} style={{ ...schemeOverlayBaseStyle, left: `${p.left}%`, top: `${p.top}%`, width: `${p.width}%`, height: `${p.height}%`, fontSize: 14 }}>{v}</div>; })}</div></div></div><div style={{ flex: 1, minWidth: 280 }}><div style={{ fontWeight: 900, marginBottom: 8 }}>Altos</div><Row>{[0,1,2].map((i) => <Field key={`alto-${i}`} label={`Alto ${i + 1} (mm)`}><Input value={form.esquema?.alto?.[i] || ""} onChange={(v) => setForm((prev) => updateSchemeValue(prev, "alto", i, v))} style={{ width: "100%" }} /></Field>)}</Row><div className="spacer" /><div style={{ fontWeight: 900, marginBottom: 8 }}>Anchos</div><Row>{[0,1,2].map((i) => <Field key={`ancho-${i}`} label={`Ancho ${i + 1} (mm)`}><Input value={form.esquema?.ancho?.[i] || ""} onChange={(v) => setForm((prev) => updateSchemeValue(prev, "ancho", i, v))} style={{ width: "100%" }} /></Field>)}</Row></div></div>{renderDynamicSectionFields({ sectionKey: "esquema_medidas", fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor })}</Section>
      <Section title={SECTION_LABELS.revestimiento}><Row><Field label="Tipo revestimiento"><select value={form.tipo_revestimiento_comercial || ""} onChange={(e) => setForm({ ...form, tipo_revestimiento_comercial: e.target.value })} style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ddd" }}><option value="">Seleccione…</option><option value="PVC">PVC</option><option value="Madera">Madera</option><option value="Aluminio">Aluminio</option><option value="chapa">Chapa</option><option value="otros">Otros</option></select></Field><Field label="Fabricante revestimiento"><Input value={form.fabricante_revestimiento || ""} onChange={(v) => setForm({ ...form, fabricante_revestimiento: v })} style={{ width: "100%" }} /></Field><Field label="Color revestimiento"><Input value={form.color_revestimiento || ""} onChange={(v) => setForm({ ...form, color_revestimiento: v })} style={{ width: "100%" }} /></Field><Field label="Color sistema"><Input value={form.color_sistema || ""} onChange={(v) => setForm({ ...form, color_sistema: v })} style={{ width: "100%" }} /></Field></Row><div className="spacer" /><Row><Field label="Listones"><Input value={form.listones || ""} onChange={(v) => setForm({ ...form, listones: v })} style={{ width: "100%" }} /></Field><Field label="Lucera"><YesNo value={form.lucera} onChange={(v) => setForm({ ...form, lucera: v })} /></Field><Field label="Cant. de luceras"><Input value={form.lucera_cantidad || ""} onChange={(v) => setForm({ ...form, lucera_cantidad: v })} style={{ width: "100%" }} disabled={!form.lucera} /></Field><Field label="Posición de lucera"><Input value={form.lucera_posicion || ""} onChange={(v) => setForm({ ...form, lucera_posicion: v })} style={{ width: "100%" }} disabled={!form.lucera} /></Field></Row>{renderDynamicSectionFields({ sectionKey: "revestimiento", fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor })}</Section>
      <Section title={SECTION_LABELS.puerta_estructura}><Row><Field label="Puerta"><YesNo value={form.puerta} onChange={(v) => setForm({ ...form, puerta: v })} /></Field><Field label="Posición de la puerta"><Input value={form.posicion_puerta || ""} onChange={(v) => setForm({ ...form, posicion_puerta: v })} style={{ width: "100%" }} disabled={!form.puerta} /></Field><Field label="Parantes cantidad"><Input value={form.parantes?.cant || ""} onChange={(v) => setForm({ ...form, parantes: { ...(form.parantes || {}), cant: v } })} style={{ width: "100%" }} /></Field><Field label="Parantes distribución"><Input value={form.parantes?.distribucion || ""} onChange={(v) => setForm({ ...form, parantes: { ...(form.parantes || {}), distribucion: v } })} style={{ width: "100%" }} /></Field></Row><div className="spacer" /><Row><Field label="Pasador manual"><YesNo value={form.pasador_manual} onChange={(v) => setForm({ ...form, pasador_manual: v })} /></Field><Field label="Instalación"><YesNo value={form.instalacion} onChange={(v) => setForm({ ...form, instalacion: v })} /></Field><Field label="Anclaje"><select value={form.anclaje || ""} onChange={(e) => setForm({ ...form, anclaje: e.target.value })} style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ddd" }}><option value="">Seleccione…</option><option value="no">No</option><option value="lateral">Lateral</option><option value="superior">Superior</option></select></Field><Field label="Piernas"><Input value={form.piernas || ""} onChange={(v) => setForm({ ...form, piernas: v })} style={{ width: "100%" }} /></Field></Row>{renderDynamicSectionFields({ sectionKey: "puerta_estructura", fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor })}</Section>
      <Section title={SECTION_LABELS.rebajes_suelo}><Row><Field label="Rebaje"><YesNo value={form.rebaje} onChange={(v) => setForm({ ...form, rebaje: v })} /></Field><Field label="Altura de rebaje"><select value={form.rebaje_altura || ""} onChange={(e) => setForm({ ...form, rebaje_altura: e.target.value })} style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ddd" }} disabled={!form.rebaje}><option value="">Seleccione…</option><option value="75mm">75mm</option><option value="100mm">100mm</option><option value="125mm">125mm</option></select></Field><Field label="Rebaje lateral"><YesNo value={form.rebaje_lateral} onChange={(v) => setForm({ ...form, rebaje_lateral: v })} /></Field><Field label="Rebaje inferior"><YesNo value={form.rebaje_inferior} onChange={(v) => setForm({ ...form, rebaje_inferior: v })} /></Field></Row><div className="spacer" /><Row><Field label="Trampa de tierra"><YesNo value={form.trampa_tierra} onChange={(v) => setForm({ ...form, trampa_tierra: v })} /></Field><Field label="Altura trampa de tierra"><select value={form.trampa_tierra_altura || ""} onChange={(e) => setForm({ ...form, trampa_tierra_altura: e.target.value })} style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #ddd" }} disabled={!form.trampa_tierra}><option value="">Seleccione…</option><option value="2 cm">2 cm</option><option value="5 cm">5 cm</option></select></Field></Row>{renderDynamicSectionFields({ sectionKey: "rebajes_suelo", fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor })}</Section>
      <Section title={SECTION_LABELS.observaciones}><textarea value={form.observaciones || ""} onChange={(e) => setForm({ ...form, observaciones: e.target.value })} style={{ width: "100%", minHeight: 100, padding: 10, borderRadius: 10, border: "1px solid #ddd" }} />{renderDynamicSectionFields({ sectionKey: "observaciones", fieldsBySection, dynamicUi, allFields, form, setForm, isTechnical, isMedidor })}</Section>
      {!!(fieldsBySection.otros || []).filter((field) => !dynamicUi.hidden.has(field.key)).length && <Section title={SECTION_LABELS.otros}><div className="muted" style={{ marginBottom: 10 }}>Campos extra que no quedaron asignados a un sector puntual.</div><Row>{(fieldsBySection.otros || []).filter((field) => !dynamicUi.hidden.has(field.key)).map((field) => { const fieldMeta = allFields.find((item) => item.key === field.key) || field; const allowed = dynamicUi.allowedOptions[field.key]; const value = getByPath(form, field.key); const disabled = !canEditDynamicField(fieldMeta, { isTechnical, isMedidor }); return <Field key={field.key} label={field.label}>{renderDynamicInput({ field: fieldMeta, value, allowedValues: allowed, disabled, onChange: (nextValue) => setForm((prev) => setByPath(prev, field.key, nextValue)) })}</Field>; })}</Row></Section>}
      <div className="card"><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Button variant="secondary" disabled={saveM.isPending} onClick={() => saveM.mutate({ submit: false })}>{saveM.isPending ? "Guardando..." : "Guardar"}</Button><Button disabled={saveM.isPending} onClick={() => saveM.mutate({ submit: true })}>{saveM.isPending ? "Enviando..." : (isTechnical ? "Confirmar datos técnicos" : "Enviar a Técnica")}</Button>{isTechnical && <Button variant="ghost" disabled={rejectM.isPending} onClick={() => { const notes = window.prompt("Motivo de corrección:", "") || ""; if (!notes) return; rejectM.mutate(notes); }}>{rejectM.isPending ? "Devolviendo..." : "Devolver para corregir"}</Button>}</div></div>
    </div>
  );
}
