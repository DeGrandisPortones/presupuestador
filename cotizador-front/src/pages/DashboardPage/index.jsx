...
export default function DashboardPage() {
  ...
  // Vista rápida del catálogo por Sección/Etiqueta (para validar el mapeo Tag → Sección)
              ...
              <div style={{ flex: 2, minWidth: 520, display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    {filteredQuotes.map((q) => (
                      <div key={q.id} style={{ border: "1px solid #eee", padding: 10, borderRadius: 10 }}>
                        ...
                        <div className="muted" style={{ fontSize: 12 }}>
                          {q.created_at ? new Date(q.created_at).toLocaleString() : ""} · {q.created_by_role}
                        </div>
                        {!!(q.sections || []).length && (
                          <div className="muted" style={{ marginTop: 6 }}>
                            Secciones: <b>{(q.sections || []).join(", ")}</b>
                          </div>
                        )}
                        {!!(q.tags || []).length && (
                          <div className="muted" style={{ marginTop: 6 }}>
                            Tags: {(q.tags || []).slice(0, 12).join(", ")}
                            {(q.tags || []).length > 12 ? "…" : ""}
                          </div>
                        )}
                      </div>
                ...
                <div className="card">
                  ...
                  <div className="muted" style={{ marginBottom: 10 }}>
                    Esto es para validar el mapeo <b>Etiqueta → Sección</b>. Si un producto no aparece:
                    <br />• Puede estar filtrado por precio (si en la lista de precios queda en $0 no lo traemos)
                    <br />• Puede existir una etiqueta duplicada con el mismo nombre (mapeaste una, el producto tiene otra)
                  </div>
                    ...
                    {filteredProductsForData.map((p) => (
                      <div key={p.id} style={{ border: "1px solid #eee", padding: 10, borderRadius: 10 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "baseline" }}>
                          <div style={{ fontWeight: 800, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        ...
                        <div className="muted" style={{ fontSize: 12 }}>
                          ID: {p.id}{p.code ? ` · ${p.code}` : ""}{p.alias ? ` · alias: ${p.alias}` : ""}
                        </div>

                        {!!(p.tags || []).length && (
                          <div className="muted" style={{ marginTop: 6 }}>
                            Tags: {(p.tags || []).slice(0, 14).join(", ")}{(p.tags || []).length > 14 ? "…" : ""}
                          </div>
                        )}
                        {!!(p.sections || []).length && (
                          <div className="muted" style={{ marginTop: 6 }}>
                            Secciones: <b>{(p.sections || []).join(", ")}</b>
                          </div>
                        )}
                      </div>
                    ))}
