...
function isMarcoComplete(door) { const r = door?.record || {}; return !!safe(r?.end_customer?.name) && !!safe(r?.end_customer?.phone) && !!safe(r?.end_customer?.address) && !!safe(r?.supplier_odoo_partner_id) && Number(r?.sale_amount || 0) > 0 && Number(r?.purchase_amount || 0) > 0; }
...
export default function PuertaPanelPage() {
  ...
  const selectedMode = safe(door?.record?.fulfillment_mode);
  const canSubmit = canSellerEdit && marcoReady && ipanelReady && door?.status === "draft";
  ...
  return (
    <div className="container">
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Button variant={selectedMode === "acopio" ? "primary" : "ghost"} disabled={!canSubmit || submitM.isPending} onClick={() => submitM.mutate("acopio")}>Enviar a Acopio</Button>
          <Button variant={selectedMode === "produccion" ? "primary" : "ghost"} disabled={!canSubmit || submitM.isPending} onClick={() => submitM.mutate("produccion")}>Enviar a Producción</Button>
        </div>
      </div>
    </div>
  );
